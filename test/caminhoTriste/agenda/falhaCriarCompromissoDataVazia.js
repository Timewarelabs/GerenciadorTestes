...
async function falhaCriarCompromissoDataVazia(driver) {
    console.log("Iniciando teste: Compromisso - Data vazia");

    await allure.step("Executando cenário: Data vazia", async (ctx) => {
        const agora = new Date();
        const horaString = "1300";
        const titulo = `Teste Sad Path - Data Vazia - ${agora.toLocaleTimeString('pt-BR')}`;

        // Navegação (Resumida para brevidade, mas completa na execução)
        ...
        // Data fica vazia
        await digitarLentamente(inputHora, horaString, driver);
        ...
        assert.ok(estaDesabilitado, `Botão Salvar deveria estar desabilitado (Data vazia)`);
...
export { falhaCriarCompromissoDataVazia };