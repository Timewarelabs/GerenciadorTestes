...
async function falhaCriarCompromissoHoraVazia(driver) {
    console.log("Iniciando teste: Compromisso - Hora vazia");

    await allure.step("Executando cenário: Hora vazia", async (ctx) => {
        const agora = new Date();
        const amanha = new Date(agora);
        amanha.setDate(amanha.getDate() + 1);
        const dataString = amanha.toLocaleDateString('pt-BR');
        const titulo = `Teste Sad Path - Hora Vazia - ${agora.toLocaleTimeString('pt-BR')}`;
        ...
        assert.ok(estaDesabilitado, `Botão Salvar deveria estar desabilitado (Hora vazia)`);
...
export { falhaCriarCompromissoHoraVazia };