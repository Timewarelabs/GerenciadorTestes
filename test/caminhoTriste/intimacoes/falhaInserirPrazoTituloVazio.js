...
async function falhaInserirPrazoTituloVazio(driver) {
  console.log("Tentando inserir um prazo com titulo vazio...");
  allure.parameter("Descrição", "Tentando inserir um prazo com titulo vazio...");
  ...
  const cenarios = [
      { desc: "Título vazio", titulo: "", data: dataAmanha, hora: horaAtual },
  ];
  ...
  for (const cenario of cenarios) {
    await allure.step(`Executando cenário: ${cenario.desc}`, async (ctx) => {
        try {
            ...
            if (encontrouErro) {
                console.log('Mensagem de erro (Título) identificada com sucesso.');
                await tirarPrint(driver, "Validacao_erro_titulo_sucesso");
                await ctx.parameter("Status", "200");
                allure.parameter("Descrição", `Deveria aparecer a mensagem para inserir informações no caso: ${cenario.desc}`);
            } else {
                await tirarPrint(driver, "Erro_validacao_titulo_ausente");
  ...
  console.log("Tentando salvar um prazo com titulo vazio finalizado.");
}

export { falhaInserirPrazoTituloVazio };