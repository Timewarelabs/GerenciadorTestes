...
async function AtualizarEmailEmpresa(driver) {
  ...
  await allure.step("Marcando checkbox de aceite", async (ctx) => {
    try {
      ...
      await ctx.parameter("Status", "400");
      await CapturaTela(driver, "Erro_ao_marcar_checkbox");
  ...
  await allure.step("Abrindo o editor de endereço", async (ctx) => {
    try {
      ...
      await CapturaTela(driver, "Erro_abrir_editor_endereco");
  ...
  await allure.step("Atualizando o campo de email", async (ctx) => {
    try {
      ...
      await CapturaTela(driver, "Antes_de_localizar_campo_email");
      ...
      await CapturaTela(driver, "Campo_email_encontrado");
      ...
      await CapturaTela(driver, "Apos_clicar_campo_email");

      await driver.actions()
        .sendKeys(Key.TAB, Key.TAB, "emailAtualizado@gmail.com", Key.ENTER)
        .perform();
      await CapturaTela(driver, "Apos_preencher_email");
  ...
  await allure.step("Clicando no botão de atualizar", async (ctx) => {
    try {
      ...
      await CapturaTela(driver, "Erro_clicar_atualizar");
