...
async function validarFiltroTarefas(driver) {
    try {
        console.log("Iniciando fluxo de filtro da agenda para tarefas...");
        ...
        await allure.step("Marcando o checkbox de Tarefas", async (ctx) => {
            try {
                const checkboxTarefas = await driver.wait(until.elementLocated(By.css('[data-testid="checkbox-tarefas"]')), 5000);
                const estaChecado = await checkboxTarefas.getAttribute("aria-checked");
                if (estaChecado !== "true") await checkboxTarefas.click();
                await driver.sleep(2000);
                await ctx.parameter("Status", "Sucesso");
            } catch (erro) {
                await ctx.parameter("Status", "Erro");
                await tirarPrint(driver, "erro_ao_marcar_checkbox_tarefas");
                assert.fail("Erro ao marcar o checkbox de Tarefas");
            }
        });

        await allure.step("Validando exibição de tarefas na agenda", async (ctx) => {
            const eventosTarefa = await driver.findElements(By.css('[data-testid*="tarefa"]'));
            assert.ok(eventosTarefa.length > 0, "Nenhuma tarefa encontrada após aplicar o filtro");
            await ctx.parameter("Status", "Sucesso");
        });

        await allure.step("Validando que apenas tarefas estão visíveis na agenda", async (ctx) => {
            const tiposInvalidos = ["compromisso", "intimacao", "prazo", "vencimento"];
            ...
            const elementosTarefa = await driver.findElements(By.css('[data-testid*="tarefa"]'));
            let temTarefaVisivel = false;

            for (const tarefa of elementosTarefa) {
            ...
            assert.ok(temTarefaVisivel, "Nenhuma tarefa visível encontrada após aplicar o filtro");
            await ctx.parameter("Status", "Sucesso");
        });

        console.log("Fluxo de filtro da agenda para tarefas finalizado com sucesso.");
