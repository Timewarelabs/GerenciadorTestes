...
async function validarFiltroPrazoIntimacoes(driver) {
    try {
        console.log("Iniciando fluxo de filtro da agenda para prazo de intimações...");
        ...
        await allure.step("Marcando o checkbox de Prazo de Intimações", async (ctx) => {
            try {
                const checkboxPrazos = await driver.wait(until.elementLocated(By.css('[data-testid="checkbox-prazos-de-intimacoes"]')), 5000);
                const estaChecado = await checkboxPrazos.getAttribute("aria-checked");
                if (estaChecado !== "true") await checkboxPrazos.click();
                await driver.sleep(2000);
                await ctx.parameter("Status", "200");
            } catch (erro) {
                await ctx.parameter("Status", "400");
                await tirarPrint(driver, "erro_ao_marcar_checkbox_prazos");
                assert.fail("Erro ao marcar o checkbox de Prazos de Intimações");
            }
        });

        await allure.step("Validando exibição de prazo de intimações na agenda", async (ctx) => {
            const eventosPrazo = await driver.findElements(By.css('[data-testid*="prazo"]'));
            assert.ok(eventosPrazo.length > 0, "Nenhum prazo de intimações encontrado após aplicar o filtro");
            await ctx.parameter("Status", "200");
        });

        await allure.step("Validando que apenas prazos de intimações estão visíveis na agenda", async (ctx) => {
            const tiposInvalidos = ["tarefa", "intimacao", "compromisso", "vencimento"];
            ...
            const elementosPrazo = await driver.findElements(By.css('[data-testid*="prazo"]'));
            let temPrazoVisivel = false;

            for (const tarefa of elementosPrazo) {
                if (await tarefa.isDisplayed()) {
                    temPrazoVisivel = true;
                    break;
                }
            }

            assert.ok(temPrazoVisivel, "Nenhum prazo visível encontrado após aplicar o filtro");
            await ctx.parameter("Status", "200");
        });

        console.log("Fluxo de filtro da agenda para prazos de intimações finalizado com sucesso.");
...
export { validarFiltroPrazoIntimacoes };