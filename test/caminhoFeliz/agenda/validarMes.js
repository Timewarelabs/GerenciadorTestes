...
async function validarMes(driver) {
    try {
        console.log("Iniciando fluxo de seleção de mês...");
        ...
        await allure.step("Selecionando a opção 'Mês'", async (ctx) => {
            try {
                const opcaoMes = await driver.wait(until.elementLocated(By.css('[data-testid="select_range_month"]')), 5000);
                await opcaoMes.click();
                await ctx.parameter("Status", "Sucesso");
            } catch (erro) {
                await ctx.parameter("Status", "Erro");
                await tirarPrint(driver, "Erro_ao_selecionar_opcao_mes");
                assert.fail("Erro ao selecionar a opção de mês");
            }
        });

        console.log("Seleção de período 'Mês' concluída com sucesso!");
        await driver.get(`${obterBaseUrl()}/home`);

    } catch (erro) {
        console.error("Erro geral ao executar fluxo de mês:", erro);
        await tirarPrint(driver, "Erro_geral_fluxo_mes");
        throw erro;
    }
}
